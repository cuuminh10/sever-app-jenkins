...
namespace gmc_api.Controllers
{
    ...
    [ApiController]
    [Route("so")]
    public class ARSOController : ControllerBase
    {
        private readonly ISimpleService _service;
        private readonly ISystemCommonService _commonService;

        public ARSOController(ISimpleService service, ISystemCommonService commonService)//, IProductionOrderService producservice)
        {
            _service = service;
        ...
        public IActionResult GetData([FromQuery] ApproveSearchBase searchCondition)
        {
            ...
            if (searchCondition.isApprove == 0)
            {
                ...
                var result = _service.getSOMyData(userInfo.UserName, searchCondition, empInfo == null ? -1 : empInfo.employeeId);
                return Ok(result);
            }
            else
            {
                ...
                return Ok(_service.getSOApproveData(userInfo.UserName, searchCondition));
            }
        }
        ...
        {
            return Ok(_service.getDetailSOItems(id));
        }
    }
}
