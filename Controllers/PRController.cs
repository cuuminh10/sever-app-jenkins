...
namespace gmc_api.Controllers
{
    ...
    public class PRController : ControllerBase
    {
        ...
        public PRController(ISimpleService service, ISystemCommonService commonService)//, IProductionOrderService producservice)
        {
            _service = service;
        ...
        public IActionResult GetData([FromQuery] ApproveSearchBase searchCondition)
        {
            ...
            if (searchCondition.isApprove == 0)
            {
                ...
                var result = _service.getAPPRMyData(userInfo.UserName, searchCondition, empInfo == null ? -1 : empInfo.employeeId);
                return Ok(result);
            }
            else
            {
                ...
                return Ok(_service.getAPPRApproveData(userInfo.UserName, searchCondition));
            }
        }
        ...
        {
            return Ok(_service.getDetailAPPRItems(id));
        }
    }
}
